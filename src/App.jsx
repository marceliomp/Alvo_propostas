      ...
      {step === "setup" ? (
        ...
        <main className="mx-auto max-w-7xl p-6">
          <div ref={resultRef} className="paper mx-auto space-y-8">
            <section className="page bg-white/95 border border-slate-200/80 rounded-[28px] shadow-xl p-10 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm">
                  <h3 className="text-sm font-semibold text-slate-600 mb-3 uppercase tracking-wide">Cliente</h3>
                  <DataRow k="Nome" v={data.cliente} />
                  <DataRow k="Telefone" v={data.clientePhone} />
                  <DataRow k="E-mail" v={data.clienteEmail} />
                </div>
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm">
                  <h3 className="text-sm font-semibold text-slate-600 mb-3 uppercase tracking-wide">Empreendimento</h3>
            ...
            <section className="page bg-white/95 border border-slate-200/80 rounded-[28px] shadow-xl p-10 space-y-6">
              <div className="flex flex-wrap items-end justify-between gap-4">
                <h3 className="text-lg font-semibold text-slate-800 tracking-tight">Resumo &amp; Condições</h3>
                <div className="text-xs text-gray-500 uppercase tracking-wider">Visão rápida para decisão</div>
              </div>
              <div className="grid gap-6 xl:grid-cols-[1.45fr_1fr]">
                <div className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm">
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-3">Resumo executivo</h4>
                ...
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-slate-100 p-5 shadow-sm space-y-3 text-sm">
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Quadro de investimento</h4>
                  ...
                  <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-3 py-2 text-emerald-800 font-semibold flex justify-between">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <span className="text-[11px] text-gray-500">Percentuais sobre o fluxo do cliente</span>
                </div>
                <FluxoResumoCards items={resumoFluxo} columns="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-gray-600">
                  <div className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 bg-slate-50">
                    ...
                    <span className="font-semibold text-slate-800">{brl(totalFluxoCliente)}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 bg-slate-50">
                    <span>Pagamento total (cliente + banco)</span>
                    <span className="font-semibold text-slate-800">{brl(totalFluxoGeral)}</span>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="page bg-white/95 border border-slate-200/80 rounded-[28px] shadow-xl p-10 space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="text-lg font-semibold text-slate-800 tracking-tight">Fluxo &amp; Cenários</h3>
                <button
                  onClick={() => setShowFluxoDetalhado((prev) => !prev)}
                  className="px-4 py-1.5 rounded-full border border-emerald-200 bg-emerald-50 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 transition"
                  type="button"
              ...
              <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
                <div className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm space-y-4">
                  <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3 text-xs text-gray-600 leading-5">
                    <div>Pagamento do cliente (sem financiamento): <strong>{brl(valores.totalFluxoSemFin)}</strong></div>
                    <div>Pagamento total (cliente + banco): <strong>{brl(valores.totalJaSomado)}</strong></div>
                    {valores.schedule.length > 0 && (
                      <div>
                        {valores.schedule.length} lançamentos · {valores.schedule[0].data.toLocaleDateString("pt-BR")} a {" "}
                        {valores.schedule[valores.schedule.length - 1].data.toLocaleDateString("pt-BR")}
                      </div>
                    )}
                  </div>
                  <div className="rounded-xl border border-slate-100 overflow-hidden">
                    {showFluxoDetalhado ? (
                      <table className="w-full text-[11px]">
                        <thead>
                          <tr className="bg-slate-100 text-slate-600">
                            ...
                            <th className="px-2 py-1 text-right">% do fluxo</th>
                          </tr>
                        </thead>
                        <tbody>
                          {valores.schedule.map((p, i) => (
                            <tr key={`${p.tipo}-${i}`} className="border-t border-slate-100 last:border-b-0">
                              <td className="px-2 py-1">{i + 1}</td>
                              <td className="px-2 py-1">{p.data.toLocaleDateString("pt-BR")}</td>
                              <td className="px-2 py-1">{p.tipo}</td>
                              <td className="px-2 py-1 text-right">{brl(p.valor)}</td>
                              <td className="px-2 py-1 text-right">{brl(p.acumulado)}</td>
                              <td className="px-2 py-1 text-right">{p.percentual.toFixed(2)}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      <div className="p-4 text-xs text-gray-600 space-y-2">
                        <p>
                          Veja o fluxo parcelado clicando em <strong>"Ver fluxo mês a mês"</strong>. Todas as parcelas e
                          percentuais estão prontas para conferência.
                        </p>
                        <p className="text-[11px] text-gray-500">
                          Fórmulas: Total = Entrada + Obra + Reforços + (Chaves à vista ou pós-chaves). Acumulado(i) =
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-emerald-100 p-5 text-[12px] shadow-sm">
                    <h4 className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-3">Cenário 1 — Revenda</h4>
                    <table className="w-full">
                      <tbody className="divide-y divide-emerald-200/60">
                        ...
                        <tr>
                          <td className="px-3 py-2 font-bold text-emerald-800">ROI</td>
                          <td className="px-3 py-2 font-bold text-emerald-800 text-right">{pct(cenario1.roi)}</td>
                        </tr>
                        <tr className="bg-emerald-100/70">
                          <td className="px-3 py-2 font-bold text-emerald-900">ROAS</td>
                          <td className="px-3 py-2 font-bold text-emerald-900 text-right">{pct(cenario1.roas)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  <div className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 via-white to-blue-100 p-5 text-[12px] shadow-sm space-y-3">
                    <h4 className="text-xs font-semibold uppercase tracking-wide text-blue-700">Cenário 2 — Short Stay (5 anos)</h4>
                    <div className="rounded-xl border border-blue-100 bg-white/80 p-3 text-[11px] leading-5 space-y-1">
                      <p className="font-semibold text-blue-700">Parâmetros principais</p>
                      <p>• ADR: {brl(cenario2.adrDiaria)}</p>
                      <p>
                        • Ocupação: {data.ocupacao}% ≈ {Math.round(((data.ocupacao || 0) * 30) / 100)} diárias/mês
                      </p>
                      <p>• Receita bruta: {brl(cenario2.receitaMensalBruta)}/mês</p>
                      <p>• Custos: {data.custosOperacionais}%</p>
                      <p>
                        • <strong>Líquido: {brl(cenario2.aluguelLiquido)}/mês</strong>
                      </p>
                    </div>
                    <table className="w-full">
                      <tbody className="divide-y divide-blue-200/60">
                        ...
                        <tr className="bg-blue-200/70">
                          <td className="px-3 py-2 font-bold text-blue-900">RETORNO TOTAL</td>
                          <td className="px-3 py-2 font-bold text-blue-900 text-right">{brl(cenario2.retornoTotal)}</td>
                        </tr>
                        <tr className="bg-emerald-50/80">
                          <td className="px-3 py-2 font-bold text-emerald-800">ROI</td>
                          <td className="px-3 py-2 font-bold text-emerald-800 text-right">{pct(cenario2.roi)}</td>
                        </tr>
                        <tr className="bg-emerald-100/80">
                          <td className="px-3 py-2 font-bold text-emerald-900">ROAS</td>
                          <td className="px-3 py-2 font-bold text-emerald-900 text-right">{pct(cenario2.roas)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              {(data.arquivoOriginal || imagensGaleria.length > 0) && (
                <div className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm space-y-4">
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Materiais do empreendimento</h4>
                  {data.arquivoOriginal && (
                    <div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3 flex flex-wrap items-center justify-between gap-3 text-xs">
                      <div>
                        <p className="font-semibold text-slate-700">Arquivo original</p>
                        <p className="text-[11px] text-gray-500">{data.arquivoOriginal.name}</p>
                      </div>
                      <a
              ...
              <div className="border-t border-slate-200 pt-4 text-[11px] text-gray-500 leading-5 space-y-1">
                <p className="italic">* Estimativas baseadas em projeções de mercado. ROI = retorno sobre o valor total; ROAS = retorno sobre o investimento real.</p>
                <p>* Formas de pagamento sujeitas a atualização por <strong>CUB (período de obras)</strong> e <strong>IGP-M + 1%</strong> após a entrega das chaves.</p>
                <p>© {new Date().getFullYear()} Alvo BR — {data.company} · {data.phone} · {data.email}</p>
              </div>
            </section>
          </div>
        </main>
  ...
  return (
    <div className={columns}>
      {items.map((item) => (
        <div
          key={item.key}
          className={`rounded-2xl border px-4 py-3 text-xs leading-5 shadow-sm transition ${
            item.destaque
        ...
        >
          <div
            className={`text-[10px] font-semibold uppercase tracking-[0.2em] ${
              item.destaque ? "text-emerald-700" : "text-slate-500"
            }`}
          >
            {item.label}
          </div>
          <div className="text-sm font-semibold text-slate-900">{brl(item.valor)}</div>
          <div className={`text-[11px] ${item.destaque ? "text-emerald-700" : "text-slate-500"}`}>
            {pct(item.percentual)} {item.contexto ? `do ${item.contexto}` : "do total"}
          </div>
          {item.detalhe ? (
            <div className={`text-[11px] ${item.destaque ? "text-emerald-700/80" : "text-slate-400"}`}>
              {item.detalhe}
            </div>
          ) : null}
        </div>
...
function KPI({ title, value, highlight, subValue }) {
  return (
    ...
    >
      ...
      {subValue && <div className="text-[11px] text-slate-500 leading-5 mt-2">{subValue}</div>}
    </div>
  );
}
