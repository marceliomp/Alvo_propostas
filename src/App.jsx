...
export default function App() {
  ...
  // The favicon é carregado diretamente pelo index.html a partir do arquivo estático, sem necessidade de override em runtime.
    ...
    setData((current) => {
      ...
      if (current.chavesForma === "posConstrutora") {
        if (current.chavesExtraQuando !== "pos_chaves") {
          if (next === current) next = { ...current };
          next.chavesExtraQuando = "pos_chaves";
        }
        if (!current.posBalaoFrequenciaMeses) {
          if (next === current) next = { ...current };
          next.posBalaoFrequenciaMeses = sample.posBalaoFrequenciaMeses;
        }
        return next;
      }
      if (current.chavesExtraQuando === "pos_chaves" || current.posBalaoValor || current.posBalaoQuantidade) {
        if (next === current) next = { ...current };
        next.chavesExtraQuando = "na_entrega";
        next.posBalaoValor = 0;
        next.posBalaoQuantidade = 0;
        next.posBalaoFrequenciaMeses = current.posBalaoFrequenciaMeses || sample.posBalaoFrequenciaMeses;
      }
  ...
  const valores = useMemo(() => {
    ...
    const posBalaoQuantidade = Math.max(0, Math.floor(Number(data.posBalaoQuantidade || 0)));
    const posBalaoValor = Math.max(0, Number(data.posBalaoValor || 0));
    const posBalaoFreq = Math.max(1, Math.floor(Number(data.posBalaoFrequenciaMeses || 1)));
    const posBalaoAtivo = data.chavesForma === "posConstrutora" && posBalaoQuantidade > 0 && posBalaoValor > 0;
    const posBalaoTotal = posBalaoAtivo ? posBalaoQuantidade * posBalaoValor : 0;
    ...
    const totalPosChaves = totalPosChavesBase + (chavesExtraPos ? chavesExtraValor : 0) + posBalaoTotal;
    ...
    if (posBalaoAtivo) {
      for (let i = 0; i < posBalaoQuantidade; i++) {
        const deslocamento = entradaParcelas + parcelasObra + 1 + i * posBalaoFreq;
        schedule.push({
          tipo: `Balão pós-chaves ${i + 1}/${posBalaoQuantidade}`,
          data: addMonths(baseDate, deslocamento),
          valor: posBalaoValor,
          responsavel: "inquilino",
        });
  ...
  const cenario2 = useMemo(() => {
    ...
    the const rendaAcumulada = aluguelLiquido * mesesOperacao;
  ...
  const fluxoResumo = useMemo(() => {
    ...
    if (valores.totalPosChaves > 0) {
      itens.push({
        ...
        detalhe:
          data.chavesForma === "posConstrutora"
            ? `${Math.max(1, Number(data.chavesPosParcelas || 0))}x de ${brl(valores.chavesTotal / Math.max(1, Number(data.chavesPosParcelas || 1)))}`
            : "Quitação bancada pela operação de aluguel",
        destaque: true,
      });
    }

    if (valores.posBalaoTotal > 0) {
      itens.push({
        key: "pos_balao",
        label: "Balões pós-venda",
        valor: valores.posBalaoTotal,
        percentual: totalShortStay > 0 ? (valores.posBalaoTotal / totalShortStay) * 100 : 0,
        contexto: "fluxo do short stay",
        detalhe: `${data.posBalaoQuantidade}x de ${brl(data.posBalaoValor)} a cada ${data.posBalaoFrequenciaMeses}m`,
        destaque: true,
      });
    }
  ...
  }, [valores, data.chavesForma, data.chavesPosParcelas, data.balaoQuantidade, data.balaoValor, data.balaoFrequenciaMeses, data.posBalaoQuantidade, data.posBalaoValor, data.posBalaoFrequenciaMeses]);
  ...
  const comparativosInvestimentos = useMemo(() => {
    ...
    const diffInMonths = (inicio, fim) => (fim.getFullYear() - inicio.getFullYear()) * 12 + (fim.getMonth() - inicio.getMonth());
  ...
  const saldoTitle =
    valores.saldoACompor > 0.5 ? "Saldo a compor" : valores.saldoACompor < -0.5 ? "Excedente (sobra)" : "Saldo em aberto";
  ...
  const resumoKPIs = [
    ...
    pagamentoShortStay ? { title: "Quitação via short stay (pós-obra)", value: brl(pagamentoShortStay) } : null,
    ...
    <div className="min-h-screen theme-wrapper">
      ...
      {step === "setup" ? (
        <main className="mx-auto max-w-7xl p-6 space-y-6 text-[color:var(--petrol-black)]">
          {/* ... resto do formulário de setup, identico ao trecho enviado anteriormente ... */}
        </main>
      ) : (
        <main className="mx-auto max-w-7xl p-6 text-[color:var(--petrol-black)]">
          {/* ... conteúdo da página de resultados, conforme trecho completo acima ... */}
        </main>
      )}
      ...
      <style>{`
        ...
        /* ... restante das regras CSS inalteradas ... */
      `}</style>
    </div>
  );
}
...
/* Demais componentes auxiliares (Card, Input, DataRow, KPI, TR) permanecem iguais ao trecho completo acima */
