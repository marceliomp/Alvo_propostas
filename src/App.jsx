...
export default function App() {
  ...
  const valores = useMemo(() => {
    ...
    // Reforços (sempre dentro da obra)
    if (qRef > 0 && vRef > 0) {
    ...
    if (data.chavesForma === "posConstrutora" && chavesTotal > 0) {
      ...
      for (let i = 1; i <= pcs; i++) {
        ...
        schedule.push({
          tipo: `Pós-chaves ${i}/${pcs}`,
          data: d,
          valor: chavesTotal / Math.max(pcs, 1),
        });
      }
    }
    ...
    const totalFluxoSemFin =
      ...
      totalObra +
      totalReforcos +
      (data.chavesForma === "avista" ? chavesTotal : totalPosChaves);
  ...
  const buildFluxosBase = (incluirChaves = true) => {
    ...
    // Reforços (no mês da obra correspondente)
    ...
    if (incluirChaves) {
      if (data.chavesForma === "avista") fluxos.push(-valores.chavesTotal);
        ...
        const pcs = Number(data.chavesPosParcelas || 0);
        for (let i = 0; i < pcs; i++)
          fluxos.push(-(valores.chavesTotal / Math.max(pcs, 1)));
      } else {
        // financiamento: não conta como saída de caixa
  ...
  const handleNumeric = (k) => (e) =>
    setData((d) => ({ ...d, [k]: currencyToNumber(e.target.value) }));
  ...
  const savePDF = async () => {
    ...
    const file = `Proposta_Alvo_${(data.cliente || "cliente")
      .replace(/\s+/g, "_")
      .replace(/[^\w_-]/g, "")}.pdf`;
    pdf.save(file);
  };
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              Página {step === "setup" ? "de Edição" : "da Proposta"} · PDF
              multipágina (corrigido)
            </p>
          </div>
      ...
      {step === "setup" ? (
        ...
        <div className="mx-auto max-w-6xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border p-3 bg-slate-50">
                ...
                <div className="mt-2 text-xs text-gray-600">
                  Parcela estimada: <strong>{brl((data.entradaValor || 0) / Math.max(1, data.entradaParcelas || 1))}</strong>
                </div>
              </div>
              ...
              <div className="rounded-xl border p-3 bg-slate-50">
                ...
                <div className="mt-2 text-xs text-gray-600">
                  Total em obra: <strong>{brl((data.obraParcelaValor || 0) * (data.duranteObraParcelas || 0))}</strong>
                </div>
              </div>
              ...
              <div className="rounded-xl border p-3 bg-slate-50">
                ...
                <div className="mt-2 text-xs text-gray-600 space-y-1">
                  ...
                  {data.chavesForma === "posConstrutora" && (
                    <div>Pós-chaves em {data.chavesPosParcelas || 0}x de <strong>{brl(valores.chavesTotal / Math.max(1, data.chavesPosParcelas || 1))}</strong></div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white border rounded-2xl p-4 shadow-sm text-sm">
                ...
                <ul className="space-y-1">
                  <li>
                    {valores.entradaParcelas === 1 ? (
                      ...
                      <>
                        Entrada: <strong>{brl(valores.entradaValor)}</strong> em{" "}
                        <strong>{valores.entradaParcelas}x</strong> ({brl(valores.entradaParcela)}/parcela) — {pct(valores.entradaPercent)}
                      </>
                    )}
                  </li>
                  <li>
                    Durante a obra: <strong>{brl(valores.duranteObraTotal)}</strong> em{" "}
                    <strong>{valores.duranteObraParcelas}x</strong> ({brl(valores.duranteObraParcela)}/mês) — {pct(valores.duranteObraPercent)}
                  </li>
                  <li>
                  ...
                  <li>
                    Chaves: <strong>{brl(valores.chavesTotal)}</strong>{" "}
                    {data.chavesForma === "financiamento" ? "(financ.)" : data.chavesForma === "posConstrutora" ? `em ${data.chavesPosParcelas || 0}x` : "(à vista)"}
                  </li>
                </ul>
                ...
                <div className="mt-3 rounded-xl bg-white border p-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    <div>
                      Fluxo total (sem financiamento): <strong>{brl(valores.totalFluxoSemFin)}</strong>
                    </div>
                    {data.chavesForma === "posConstrutora" && (
                      <div>
                        Pós-chaves (total): <strong>{brl(valores.totalPosChaves)}</strong>
                      </div>
                    )}
                    {data.chavesForma === "financiamento" && (
                      <div>
                        Total financiado (banco): <strong>{brl(valores.totalFinanciado)}</strong>
                      </div>
                    )}
                    <div>
                      Subtotal até chaves: <strong>{brl(valores.totalAteChaves)}</strong>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-end">
            <button
              onClick={gerarProposta}
              className="px-4 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm"
            >
              Gerar Proposta
            </button>
          </div>
        ...
        <div className="mx-auto max-w-4xl p-4">
          <div
            ref={resultRef}
            className="paper mx-auto bg-white shadow-md rounded-2xl overflow-hidden ring-1 ring-slate-200"
          >
            {/* capa */}
            <section className="p-12 page">
              ...
              <p className="mt-4 text-[13px] text-gray-700 leading-6">
                A <strong>Alvo BR</strong> é especializada em curadoria de investimentos imobiliários, unindo dados, método e resultado.
              </p>
              <p className="mt-4 text-[11px] text-gray-500">
                Validade desta proposta: <strong>{data.validade}</strong>
              </p>
            </section>
            ...
            <section className="p-12 page page-break">
              ...
              <div className="grid grid-cols-2 gap-8 text-[13px]">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {data.chavesForma === "posConstrutora" && (
                    <KPI title="Pós-chaves (total)" value={brl(valores.totalPosChaves)} />
                  )}
                  {data.chavesForma === "financiamento" && (
                    <KPI title="Financiado (banco)" value={brl(valores.totalFinanciado)} />
                  )}
                  ...
                  <ul className="space-y-1">
                    <li>
                      ...
                      {valores.entradaParcelas === 1
                        ...
                        : `${brl(valores.entradaValor)} em ${valores.entradaParcelas}x de ${brl(valores.entradaParcela)}`}
                    </li>
                    <li>
                      Durante a obra: {brl(valores.duranteObraTotal)} em {valores.duranteObraParcelas}x de{" "}
                      {brl(valores.duranteObraParcela)}
                    </li>
                    <li>
                      Reforços: {brl(valores.reforcosTotal)}{" "}
                      {valores.qRef ? `(${valores.qRef}× de ${brl(valores.vRef)} a cada ${valores.freqRef}m)` : ""}
                    </li>
                    <li>
                      Chaves: {brl(valores.chavesTotal)}{" "}
                      {data.chavesForma === "financiamento"
                        ? "(financ.)"
                        : data.chavesForma === "posConstrutora"
                        ? `em ${data.chavesPosParcelas || 0}x`
                        : "(à vista)"}
                    </li>
                  </ul>
                </div>
            ...
            <section className="p-12 page page-break">
              ...
              <div className="grid grid-cols-2 gap-8 text-[13px]">
                <div>
                  ...
                  <DataRow
                    k="Durante a obra"
                    v={`${brl(valores.duranteObraTotal)} em ${valores.duranteObraParcelas || 0}x (${brl(
                      valores.duranteObraParcela
                    )}/mês)`}
                  />
                  <DataRow
                    k="Reforços"
                    v={`${brl(valores.reforcosTotal)}${
                      valores.qRef ? ` · ${valores.qRef}x de ${brl(valores.vRef)} a cada ${valores.freqRef}m` : ""
                    }`}
                  />
                  <DataRow
                    k="Chaves"
